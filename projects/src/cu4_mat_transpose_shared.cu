...
__global__ void kernel_mat_transpose(float* res, const float* src, int n) {
    ...
    int blk_x = blockIdx.x * blockDim.x;
    int blk_y = blockIdx.y * blockDim.y;

    bool valid =  blk_x + threadIdx.x < n
               && blk_y + threadIdx.y < n;

    if (valid)
        mat[threadIdx.y * 16 + threadIdx.x]
            = src[(blk_x + threadIdx.y) * n + (blk_y + threadIdx.x)];

    __syncthreads();

    if (valid)
        res[(blk_y + threadIdx.y) * n + (blk_x + threadIdx.x)]
            = mat[threadIdx.x * 16 + threadIdx.y];
}
